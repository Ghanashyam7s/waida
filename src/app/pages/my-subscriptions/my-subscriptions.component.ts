...
export class MySubscriptionsComponent implements OnInit {
  ...
  constructor(private _fb: FormBuilder, private router:Router, private spinner: NgxSpinnerService, private membershipService: MembershipService, private toast: ToastService, private storage: AngularFireStorage, private _matdialog: MatDialog,) { }
//private membershipService:MembershipService,private spinner: NgxSpinnerService
  ngOnInit(): void {
    ...
    .subscribe(user => {
      ...
      this.membershipService.getMyMembershipSubscriptions(this.user.uid).subscribe(data=>{
        ...
        for(let memSub of this.memberSubscription){
          console.log(memSub.days)
          if(memSub.days == 90){
            this.membershipPlan = "3 Months Membership"
          } else if(memSub.days == 180){
            this.membershipPlan = "6 Months Membership"
          }  else if(memSub.days == 270){
            this.membershipPlan = "9 Months Membership"
          } else{
            this.membershipPlan = "1 year Membership"
          }
        }
      })
